...
module Boattr
  class Sensors
    ...
    def voltage(name,address)
      ...
      @raw     = @@data[address]
      @volts = @raw * 0.015357
      return { 'name' => @name,'raw' => @raw ,'volts' => @volts.round(3) }
    end
    def current(name,address)
      ...
      return { 'name' => @name,'raw' => @raw ,'volts'=> @volts.round(3) ,'amps' => @amps.round(3)} 
    end
  end
  class Data  
    def initialize()
...
foo=Boattr::Sensors.new(brain02)
p foo.read()

brain02_sensors=[foo.voltage('bat',1), foo.current('amps',0)]

bar=Boattr::Data.new()
bar.to_db(brain02_sensors)
